...
#include "dynamixel_sdk_examples/phi.h"
...
ros::Subscriber sub_;
ros::Publisher pub_= n_.advertise<dynamixel_sdk_examples::phi>("/phi",1);
  ...
  dynamixel_sdk_examples::GetPosition::Response & res)
{
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    ...
    dynamixel_sdk_examples::phi output;
    output.phi = position;
    pub_.publish(output);
...
class SubscribeAndService
{
public:
	SubscribeAndService()
	{
          srv_ = n_.advertiseService("/get_position", getPresentPositionCallback);
	  sub_ = n_.subscribe("/set_position", 1, setPositionCallback);

	}
	
	/*void setPositionCallback(const dynamixel_sdk_examples::SetPosition& msg)
	  {
  	    uint8_t dxl_error = 0;
  	    int dxl_comm_result = COMM_TX_FAIL;
	 ...
	 bool getPresentPositionCallback(dynamixel_sdk_examples::GetPosition::Request & req,dynamixel_sdk_examples::GetPosition::Response & res)
	  {
	      ...
	      {
    	        ROS_INFO("getPosition : [ID:%d] -> [POSITION:%d]", req.id, position);
                dynamixel_sdk_examples::phi phi;
                phi.phi = position;
                pub_.publish(phi);
    	        res.position = position;
    	        return true;
